...
export class ProfilePageService {
    ...
    update_biography(new_biography) {
        const store_CurrentUser = storeCurrentUser();
        store_CurrentUser.current_user.credentials.background.biography =
            new_biography;
    }

    update_education(new_education) {
        const store_CurrentUser = storeCurrentUser();
        store_CurrentUser.current_user.credentials.background.education.splice(
            new_education.index,
            1,
            new_education.new_data
        );
    }
    ...
    add_education(new_education) {
        const store_CurrentUser = storeCurrentUser();
        store_CurrentUser.current_user.credentials.background.education.push(
            new_education
        );
    }
}
