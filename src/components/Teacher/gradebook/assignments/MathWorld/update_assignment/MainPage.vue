<template>
    <div class="main-content">
        ...
        <div class="content-title">
            <span class="class-name">New MathWorld Assignment</span>
            <v-btn rounded color="#ffff" @click="dialog.status = true">
                Discard
            </v-btn>
        </div>
    ...
    data() {
        return {
            ...
            dialog: {
                ...
                message:
                    "Do you want to discard all the operations? Changes you made so far will not be saved.",
            },
        };
    },
    ...
    methods: {
        ...
        clickUpdate() {
            this.$router.push({
                ...
                params: {
                    class_code: this.$route.params.class_code,
                    //to select assignment tab
                    assignment_tab: 2,
                },
            });
            this.$emit("SnackBarSuccess", "Assignment successfully updated");
        },

        checkAssignmentType() {
            if (this.$route.params.assignment_type == "MW") {
            } else {
                this.$router.push({ name: "PageNotFound" }).catch((err) => err);
            }
        },
    },
