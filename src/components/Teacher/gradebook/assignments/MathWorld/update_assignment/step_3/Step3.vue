...
export default {
    ...
    data() {
        return {
            ...
            items: ["ALG1-S1", "ALG2-S2", "ALG3-S3", "ALG4-S4", "ALG5-S5"],
        };
    },
    ...
    methods: {
        ...
        remove(item) {
            this.new_data.assigned_class.splice(
                this.new_data.assigned_class.indexOf(item),
                1
            );
        },
        ...
        getCurrentData() {
            var assignment = MW_UpdateAssignment.prototype.getAssignmentByCode(
                this.$route.params.assignment_code
            );
            this.new_data.range.start = assignment.set_test.date.start;
            this.new_data.range.end = assignment.set_test.date.end;

            for (
                var i = 0;
                i < assignment.set_test.assigned_classes.length;
                i++
            ) {
                this.new_data.assigned_class.push(
                    assignment.set_test.assigned_classes[i]
                );
                this.current_data.assigned_class.push(
                    assignment.set_test.assigned_classes[i]
                );
            }

            this.current_data.range.start = assignment.set_test.date.start;
            this.current_data.range.end = assignment.set_test.date.end;
        },

        rangeNoChanges() {
            if (
                JSON.stringify(this.current_data.range) ===
                JSON.stringify(this.new_data.range)
            ) {
                return true;
            } else {
                return false;
        ...
        submit() {
            return MW_UpdateAssignment.prototype.submit_Step3(
                this.formatDate(this.new_data.range.start),
                this.formatDate(this.new_data.range.end),
    ...
    computed: {
        ...
        assignedClassNoChanges() {
            var arr1 = this.current_data.assigned_class;
            var arr2 = this.new_data.assigned_class;
            var common = 0;

            for (var i = 0; i < arr2.length; i++) {
                var index = arr1.findIndex((item) => item === arr2[i]);
                if (index >= 0) {
                    common++;
                }
            }
        ...
        isValid() {
            if (
                this.hasErrors ||
                (this.rangeNoChanges() && this.assignedClassNoChanges)
            ) {
                return true;
            } else {
                return false;
        ...
        startDateErrors() {
            ...
            !this.$v.new_data.range.start.required &&
        ...
        endDateErrors() {
            ...
            !this.$v.new_data.range.end.required &&
        ...
        assignedClassErrors() {
            ...
            !this.$v.new_data.assigned_class.required &&
